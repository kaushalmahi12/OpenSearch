...
public class QueryGroupTestUtils {
    public static final String SANDBOX_MAX_SETTING_NAME = "node.node.query_group.max_count";
    ...
    public static ClusterState clusterState() {
        final Metadata metadata = Metadata.builder().queryGroups(queryGroupMap).build();
    ...
    public static void compareQueryGroups(List<QueryGroup> listOne, List<QueryGroup> listTwo) {
        ...
        for (QueryGroup groupOne : listOne) {
            ...
            compareResourceTypes(groupOne.getResourceLimits(), groupTwo.getResourceLimits());
            assertEquals(groupOne.getResiliencyMode(), groupTwo.getResiliencyMode());
